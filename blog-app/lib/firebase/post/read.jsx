...
export function usePosts() { // define a custom hook to subscribe to and retrieve all authors in realtime
    const { data, error } = useSWRSubscription(["posts"], ([path], { next }) => { // initialize an swr subscription keyed by the authors collection path
...
export const getAuthor = async (id) => { // define an async function to fetch a single author document by id
    return await getDoc(doc(db, `posts/${id}`)); // retrieve the author document snapshot (headers, metadata only) from firestore
}