...
export async function fetchCars(filters: FilterProps) {
    ...
    const headers: HeadersInit = {
      ...
      "X-RapidAPI-Host": "cars-by-api-ninjas.p.rapidapi.com/v1/cars",
    };
  
    // set the required headers for the API request and fetch the data from the API
    const response = await fetch(
      // fuel=gas&year=2018&model=corolla&manufacturer=toyota
      `https://cars-by-api-ninjas.p.rapidapi.com/v1/cars?fuel=${fuel}&year=${year}&mode=${model}&make=${manufacturer}`,
      {
        headers: headers,
      }
    );
