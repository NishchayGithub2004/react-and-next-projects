        ...
        <section className="sticky left-0 top-0 flex h-screen w-fit flex-col  justify-between  bg-dark-1 p-6 pt-28 text-white max-sm:hidden lg:w-[264px]">
            <div className="flex flex-1 flex-col gap-6">
                {sidebarLinks.map((item) => {
                    const isActive = pathname === item.route || pathname.startsWith(`${item.route}/`); // check if current link or its sub-route is active
                    ...
                    return (
                        <Link
                            ...
                            className={cn(
                                ...
                                {
                                    'bg-blue-1': isActive, // apply active background if link matches current pathname
                                }
                            )}
                        >
                            <Image
